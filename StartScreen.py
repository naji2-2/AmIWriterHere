...
class StartScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button_to_Writing = tk.Button(button_frame, text="글 쓰러가기 ▶", font=("제주고딕", 30),
                                      command=lambda: (controller.play_button_click_sound(), controller.show_frame("WritingScreen")))
        button_to_Reading = tk.Button(button_frame, text="글 보러가기 ▶", font=("제주고딕", 30),
                                      command=lambda : (controller.play_button_click_sound(), controller.show_frame("ReadingScreen")))
